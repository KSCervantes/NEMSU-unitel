...
class DustParticle {
  ...
  draw(
    ...
    const y =
      centerY + Math.sin(spiralAngle) * this.distance * 0.6 + this.yOffset;
...
export default function GalaxyBackground() {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      // Reset transform to avoid accumulating scale on resize
    ...
    const initStars = () => {
      ...
      const starCount = Math.min(
        500,
        Math.floor((canvas.width * canvas.height) / (2000 * devicePixelRatio))
      );
    ...
    const initDustParticles = () => {
      ...
      const particleCount = Math.min(
        2000,
        Math.floor((canvas.width * canvas.height) / (500 * devicePixelRatio))
      );
    ...
    const animate = () => {
      ...
      // Background gradient (blend with existing hero color)
      const gradient = ctx.createLinearGradient(0, 0, 0, canvas.height);
      gradient.addColorStop(0, "#0a0a1a");
      gradient.addColorStop(0.5, "#0b1433");
      gradient.addColorStop(1, "#0f1b4a");
      ctx.fillStyle = gradient;
      ctx.fillRect(0, 0, canvas.width, canvas.height);

      // Nebula
      drawNebula(ctx, canvas.width / devicePixelRatio / 2, canvas.height / devicePixelRatio / 2, time);
      ...
      centerGlow.addColorStop(0, "rgba(255, 255, 200, 0.2)");
    ...
    const drawNebula = (
      ...
      drawNebulaCloud(ctx, x - 100, y + 50, 300, "rgba(70, 0, 100, 0.08)", time);
      drawNebulaCloud(
        ctx,
        x + 150,
        y - 100,
